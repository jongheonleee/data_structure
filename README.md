...
## ⭐️ 목표
> ### 자바언어로 시작하는 자료구조 책 반복
> ### c 언어로 자료구조 직접 구현 및 종만북 
> ### => '자바언어로 시작하는 자료구조의 이해'(현재 : 🏃🏻) -> '열혈강의 자료구조' -> '종만북'
...
> ### 📌 07. Graph : 정점(Vertex), 간선(Edge)의 집합으로 구성 u - v
...
> ### 👉 Graph
> ### - 정점, 간선의 집합, 서로 연결되어 있거나 연결 되어 있지 않음
> ### - 트리와의 차이점, 그래프는 순회 구조 가능함, 또한 트리는 한 방향으로만 내려가는 형태
...
- 그래프 기초 용어
  - 경로 : u -> ... -> v
  - 단순 경로 : 경로 상의 정점들이 모두 다른 경로(중복 x)
  - 사이클 : u -> ... -> v, u = v
  - 연결 성분 : 정점들이 서로 연결되어 있는 부분
  - 진입 차수 : u <- v, 이때 v의 개수
  - 진출 차수 : u -> v, 이때 v의 개수
  - 가중치 그래프 : 간선에 가중치가 부여된 것
    - 가중치 : 시간, 비용, 거리 ...
  - 트리 : 사이클이 없는 그래프
  - 신장 트리 : 그래프의 모든 정점을 사이클 없이 연결하는 부분 그래프 
...
- 위상 정렬 : 사이클 없는 방향 그래프(DAG)에서 정점들을 선형 순서로 나열한 것 
